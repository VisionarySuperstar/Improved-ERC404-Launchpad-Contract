...
import "./lib/interfaces/IERC165.sol";
import "hardhat/console.sol";
import "@openzeppelin/contracts/proxy/utils/Initializable.sol";

abstract contract NewERC404Upgradeable is Initializable, IMy404 {
  using DoubleEndedQueue for DoubleEndedQueue.Uint256Deque;
    ...
    bytes32 private constant ERC404StorageLocation = 0x52c63247e1f47db19d5ce0460030c497f067ca4cebf71ba98eeadabe20bace00;

    function _getERC404Storage() private pure returns (ERC404Storage storage $) {
        assembly {
            $.slot := ERC404StorageLocation
        }
    }
    function __ERC404_init(string memory name_, string memory symbol_) internal onlyInitializing {
        __ERC404_init_unchained(name_, symbol_);
    }
    function __ERC404_init_unchained(string memory name_, string memory symbol_) internal onlyInitializing {
    ...
    function supportsInterface(
        ...
        return
        interfaceId == type(IMy404).interfaceId ||
    ...
    function setWhitelistInternal(address target_, bool state_) internal virtual {
        ERC404Storage storage $ = _getERC404Storage();
    ...
    function getWhitelist(address target_) public view returns(bool state){
        ERC404Storage storage $ = _getERC404Storage();
        state = $.erc721TransferExempt[target_] ;
        return state ;
    }
    ...
    function _getUnit() public view returns(uint256){
        ERC404Storage storage $ = _getERC404Storage();
        return $.units ;
    }
    function _setNameSymbol(string memory name_, string memory symbol_) internal virtual{
        ERC404Storage storage $ = _getERC404Storage();
        $.name = name_ ;
        $.symbol = symbol_ ;
    }
    ...
    function isApprovedForAll(
        ...
        ERC404Storage storage $ = _getERC404Storage();
        state = $.isApprovedForAll[owner_][operator_];
        return state ;
    }
    function allowance(
        address owner_,
        ...
        ERC404Storage storage $ = _getERC404Storage();
        return (value = $.allowance[owner_][spender_]);
    }
}
